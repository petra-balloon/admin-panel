...
const ReservationReports = props => {
  ...
  /* ticket/singledate-reservation */

  const getReservationDataSingleDate = async event => {
    setIsLoading(true)
    ...
    try {
      if (switch3) {
        var url = "ticket/singledate-chechIn"
      } else {
        var url = "ticket/singledate-reservation"
      }
     const targetdates= moment(singletDate).format()
      await axiosApi
        .post(`${API_URL}${url}`, { singleDate: targetdates }, config)
        .then(async response => {
          console.log(
            "this is promo data>>>>>>>>>>>>>>>>>>>>>>>",
            response.data.data
          )
          setTotalUpholdenDiscount(response.data.data.upholdenTotal)
          ...
          setTotalCheckIn(response.data.data.TotalCheckIn)
          setIsLoading(false)
        })
  ...
  const getPromoDataBetweenDate = async event => {
    ...
    const starttargetdates= moment(startDate).format()
    const endtargetdates= moment(endDate).format()

    try {
      await axiosApi
        .post(
          `${API_URL}ticket/betweendate-reservation`,
          { startDate: starttargetdates, endDate: endtargetdates },
          config
        )
        .then(async response => {
          console.log(
        ...
        <Container fluid>
          <div className="page-title-box">
            <Row className="align-items-center">
              <Col lg={2}>
                <h6 className="page-title">Reservation Report</h6>
                <Button
              ...
              <Col>
                <label>Single Date reservation</label>
                <DatePicker
              ...
              <Col>
                <Button
                  ...
                  onClick={async () => {
                    await getReservationDataSingleDate()
                  }}
                >
                  Success
                </Button>{" "}
          ...
          <Row>
                ...
                <CardBody>
                  <div className="mb-4">
                    ...
                    <h4 className="fw-medium font-size-24">
                      {switch3 && totalCheckIn} {!switch3 && totalNUmberofUpholden}
            ...
            <Col xl={3} md={6}>
            {!switch3 &&  <Card className="mini-stat bg-primary text-white">
                <CardBody>
                  <div className="mb-4">
                    <div className="float-start mini-stat-img me-4">
...
export default withTranslation()(ReservationReports)
