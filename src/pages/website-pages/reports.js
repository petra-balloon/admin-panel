...
const Reports = props => {
  ...
  const getReportDataSingleDate = async event => {
    ...
    try {
      if (switch3) {
        var url = "ticket/singledate-reports-cancel"
      } else {
        var url = "ticket/singledate-reports"
      }

      await axiosApi
        .post(`${API_URL}${url}`, { singleDate: singletDate }, config)
        .then(async response => {
          console.log(
            "this is promo data>>>>>>>>>>>>>>>>>>>>>>>",
            response.data.data
  ...
  const getReportDataBetweenDate = async event => {
    ...
    try {
      if (switch3) {
        var url = "ticket/betweendate-reports-cancel"
      } else {
        var url = "ticket/betweendate-reports"
      }
        ...
        <Container fluid>
          <div className="page-title-box">
            <Row className="align-items-center">
              ...
              <Col lg={2}>
                <div>
                  <label>Only Cancel Reports</label>
                </div>
                <Switch
              ...
              <Col>
                <div><label>Single Day</label></div>
                <Button
                  color="success"
